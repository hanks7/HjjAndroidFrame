...
public class UStatusBar {
    ...
    public static int getStatusBarHeight() {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT)
            return result;
        int resourceId = HjjApplication.getInstance().getResources().getIdentifier("status_bar_height", "dimen", "android");
        if (resourceId > 0) {
            result = HjjApplication.getInstance().getResources().getDimensionPixelSize(resourceId);
        }
        return result;
    }
